...
export class TaxonomyRepository {
  ...
  async search(query: string, rank: string, limit: number, offset: number): Promise<TaxonomySearchItem[]> {
    ...
    return rows.map((row) => ({
      ...
      representativeImage:
        this.mapRepresentativeImage(row),
    }));
  }
  ...
  async treePath(taxonId: string): Promise<TaxonomyTreeNode[]> {
    const rows = await this.prisma.$queryRawUnsafe<TaxonomyRow[]>(
      `
        WITH path_taxa AS (
          ...
          JOIN taxa parent
            ON parent.taxon_id = tc.ancestor_taxon_id
          WHERE tc.descendant_taxon_id = $1
    ...
    return rows.map((row) => ({
      ...
      speciesCount: Number(row.species_count),
      childCount: Number(row.child_count),
      representativeImage: this.mapRepresentativeImage(row),
      children: [],
    }));
  }
  ...
  private mapRepresentativeImage(row: TaxonomyRow) {
    if (!row.image_source_table || !row.image_species_id || row.image_order === null) {
      return null;
    }
      ...
      clauses.push(`
        (
          ...
          OR EXISTS (
            ...
            JOIN (${SPECIES_UNION_SQL}) species_lookup
              ON species_lookup.source_table = st.source_table
             AND species_lookup.species_id = st.species_id
            WHERE st.taxon_id = t.taxon_id
              AND (
                lower(coalesce(species_lookup.vietnamese_name, '')) LIKE lower('%' || $${index} || '%')
                OR lower(coalesce(species_lookup.scientific_name, '')) LIKE lower('%' || $${index} || '%')
              )
          )
        )
      `);
